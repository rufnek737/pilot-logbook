...
## 2026-09-22 (착륙 유형 기본값·로그북 표시)

### ✅ 착륙 유형(LDT) 기본값을 메뉴얼로 통일
경로 4개 중 **크루 커넥스 신규 가져오기만 비어 있었다**(`—` 해당없음).
수동 입력·PDF/엑셀 업로드는 이미 `M`이었음. → `_buildCcFlight`의 신규 기본값을 `'M'`으로.
기존 기록을 크루 커넥스로 갱신할 때는 이미 고른 값을 유지한다.

### ✅ 로그북 카드에 착륙 유형 표시
- 카드 푸터에 **LDT 칸 추가** (T/O · LDG 다음). 실비행만 — SIM은 착륙 유형 개념이 없다.
- 라벨은 풀어서: `MAN` / `AUTO` / `CAT2` / `CAT3` / `NPA` (`2`만 적으면 뜻이 안 보임).
- 색으로 구분: 오토·CAT II/III = accent, 비정밀(NPA) = warning, 메뉴얼 = 기본 회색.
- **상단 ALD/CAT 배지는 제거** — 같은 정보를 한 카드에 두 번 보여주게 되므로.
  배지가 다시 필요하면 `aldBadge`를 되살리면 된다.

### 검증
`npm test` 41개 통과. iPhone 폭에서 MAN(회색)·CAT3(파랑) 표시 확인.

---

## 2026-09-09 (회사 ePaper 양식 반영)

제주항공 제출 양식 **E-FLIGHT LOG(ePaper)** 와 앱 입력 폼의 형식을 맞추는 작업.

### ✅ 이륙/착륙 — 체크 방식으로 변경
- `PF / PM / 해당없음` 드롭다운 2개 → ePaper와 동일한 **T/O · L/D 체크박스**.
- 저장 형식은 기존 `toPf`/`ldgPf`(`PF`/`PM`) 유지 → 착륙 횟수·90일 커런시 통계 무영향.
- `openModal()` 프리필 루프가 체크박스도 처리하도록 확장(`el.type === 'checkbox'`).
...
- 신규 필드 `ldt`. 구형 기록은 `ldtOf()`가 `ald='Y'→A`, `'N'→M`으로 읽어 재입력 불필요.
- `ald`는 구버전 호환용으로 계속 기록(`ldt==='A' ? 'Y' : 'N'`).
- 목록 배지도 `ALD` / `CAT 2` / `CAT 3` / `NPA`로 구분.
- **가정**: 오토랜딩 통계(`statAld`, `ald90`)에 `A`뿐 아니라 `2`,`3`도 포함(`isAutoland()`).
  CAT II를 메뉴얼로 수행하는 운영이면 이 함수만 고치면 됨.

### 미결
- ePaper의 **TP**(`-,A,R,D,T`) / **S/U**(`-,S,U`) 코드 의미 미확인 → 아직 미반영.
- ePaper는 BLOCK TIME(RO/RI/BT)과 TIME IN SVC(TO/LD/AT)를 분리. 현재 앱은 블록시간만 보유.

### 검증
`npm test` 41개 통과. 폼 렌더링 육안 확인.

**롤백 지점: `e97d02d`** — 아래 사이드바 개편을 되돌리려면 이 커밋으로 reset.

---

## 2026-09-09 (네비게이션 → 전체 사이드바)

LogTen(Pilotbase)처럼 **모든 메뉴를 사이드 드로어 하나로** 통합. (선택지 중 2번)

### 배경
진입점 12개가 세 군데(상단 탭바 4 / 로그북 툴바 5 / 헤더 우측 3)로 흩어져 있었고,
툴바 버튼 5개가 아이폰에서 `flex-wrap`으로 두 줄로 접혀 비행 목록을 아래로 밀었다.

### 변경
- **탭바 제거** (`.tab-bar` 마크업 + CSS 삭제). 헤더 좌측에 햄버거(`#drawerBtn`) 추가.
- **드로어 신설** — 3개 그룹:
  - 로그북: 비행 기록 · 업로드 · 통계 · 자격 유지
  - 도구: 크루 커넥스 · 기간 합산 · 출력 · 기록 점검 · 비행 입력
  - 계정: 구독 현황 · 계정 및 도움말 · 테마 전환
- **현재 위치 표시** — 탭바가 없어졌으므로 헤더 부제(`#headerSection`)에 현재 섹션명을
  띄우고, 드로어 항목에 좌측 바 + 강조색(`.drawer-item.active`)을 준다.
- **툴바 정리** — 로그북 툴바에는 `크루 커넥스`, `+ 입력`만 남김(한 줄 고정).
- `switchTab(name)` — 두 번째 인자(버튼 엘리먼트) 제거. 호출부 3곳 정리.
- `openRangePanel()` / `openAuditPanel()` 추가 — 두 패널은 로그북 탭 안에 있으므로
  탭을 먼저 맞추고 **항상 펼친 뒤** 스크롤. (기존 `toggle*`은 메뉴에서 누르면 닫힐 수 있었음)
- 드로어도 `hasOpenScrollLockSurface()`의 스크롤 락 대상에 포함.
- `drawerRun(fn)` — 드로어를 닫는 애니메이션(200ms) 후 모달을 열어 스크롤 락 충돌 방지.

### 함정 하나
`switchTab`이 모듈 레벨 `const TAB_TITLES`를 참조하게 짰더니, 스크립트가 앞에서 중단되면
TDZ로 함수 전체가 죽었다(함수 선언은 호이스팅돼 호출은 되므로 원인이 안 보인다).
→ 맵을 함수 안으로 옮김.

### 후속 — 각 메뉴를 독립 화면으로 (링크 느낌 제거)

사이드바 항목이 "링크"처럼 느껴진다는 지적. 원인은 **기간 합산·기록 점검이 로그북 화면 안의
접이식 패널**이라, 메뉴를 눌러도 로그북 전체가 보이고 해당 위치로 스크롤만 됐던 것.

- `rangePanel` / `auditPanel`을 로그북 pane 밖으로 들어내 **독립 pane**으로 분리
  (`#tab-range`, `#tab-audit`). 드로어 항목도 `data-tab`을 가진 정식 목적지가 됨
  → 이제 다른 화면처럼 헤더 섹션명·좌측 강조 바가 동일하게 동작.
- `toggleRangePanel()` / `toggleAuditPanel()` / `openRangePanel()` / `openAuditPanel()` 제거.
  진입 시 기본 기간을 채우는 로직만 `initRangePane()`으로 남김.
- 화면 전환 시 `window.scrollTo(0,0)` — 이전 화면 스크롤 위치가 남지 않게.
- **`+ 비행 추가` FAB을 비행 기록 화면에서만** 표시. 합산·점검·통계 화면에 떠 있으면
  같은 종류의 혼선이 생김.
- **테마 전환을 헤더 원위치로 복구** (드로어에서 제거). 사용자 요청.

### 검증
- `npm test` 41개 통과.
- `.claude/launch.json` 추가 — `www/`를 로컬 서버(4173)로 띄워 실제 확인.
  `file://`/`data:` 프리뷰는 localStorage가 막혀 스크립트가 중단되므로 UI 검증에 쓸 수 없다.
- iPhone 폭(375)에서 드로어 열기/이동/닫기, 헤더 섹션명 갱신, 합산·점검 화면 확인.
- 실기기(iPhone 15 Pro Max) CLI 무선 설치로 검증.
- **TestFlight 1.1(10) 업로드** — 사이드바 개편 + ePaper 폼 반영분.

---

## 2026-09-05 (전월 비행 누락 해결 ✅ + 키보드 가림 해결 ✅)

### ✅ 8/31 비행 누락 — 해결

어제까지 미해결이던 문제를 `wrangler tail` 실시간 로그로 원인 두 겹을 모두 특정해 해결.

**원인 1 — postback 방식이 틀렸음**
- 기간 조회 버튼들이 `type="submit"`인데 `__EVENTTARGET`으로 보내고 있었음.
  ASP.NET은 submit 버튼의 **name=value 쌍**을 본문에서 찾으므로, 서버는 `dateRangeHidden`
  값만 받아 되돌려주고(응답에 그대로 echo) 정작 표는 재조회하지 않았음.
- → `ctl00$Main$periodSelect`(옵션값 `2026-08-01|2026-08-31`) + `ctl00$Main$period=Period`
  방식으로 교체 (`fetchRosterPeriod`).

**원인 2 — 날짜를 시:분까지 비교**
- 조회 창 시작이 현재 시각 기준으로 계산돼 `8/31 01:40`이 되는 바람에,
  `8/31 00:00`까지인 AUG26 옵션이 **1시간 40분 차이로 대상에서 탈락**(`targets: []`).
- 같은 이유로 `parseRosterHtml`의 ±5일 필터도 8/31을 잘라냄.
- → 공통 헬퍼 `rosterWindow()`로 **날짜 문자열(YYYY-MM-DD) 단위 비교**로 통일.

**검증 로그**
```
[PERIOD] targets: ["AUG26 (01Aug26..31Aug26)"]
[PERIOD] AUG26 → 2 ["2026-08-31","2026-08-31"]
[TRYFETCH] final: 7 (8/31 2편 + 9월 5편)
```
사용자 실기기에서 8/31 비행 정상 표시 확인. 이후 진단 로그는 모두 제거하고 재배포.

### ✅ 입력창이 키보드에 가리는 문제 — 해결

세 번의 시도 끝에 해결. 실패한 두 방법과 이유를 남겨둠:

| 시도 | 방법 | 실패 이유 |
|---|---|---|
| 1 | `scrollIntoView({block:'center'})` | 웹뷰가 안 줄어드니 브라우저는 "이미 보인다"고 판단해 스크롤 안 함 |
| 2 | `visualViewport` 높이 계산 | `resize:"none"`이라 visualViewport도 안 줄어듦 → 계산값 항상 0 |
| 3 ✅ | `@capacitor/keyboard`의 `keyboardWillShow` 이벤트 | 네이티브에서 키보드 높이를 직접 수신 |

- 핵심: `resize:"none"`에서는 **웹뷰가 키보드 존재를 전혀 알 수 없다.** 유일한 경로가 플러그인 이벤트.
- 키보드 높이를 `--keyboard-inset` CSS 변수 + `body.keyboard-open`으로 반영해
  (a) 가려진 만큼 `padding-bottom`으로 **스크롤 여유 확보**, (b) 포커스된 입력창 하단이
  가시 영역 아래면 그만큼만 스크롤.
- 입력창 있는 화면 전수 적용: 비행 입력/수정(비고 포함), 기존 경력, 크루 커넥스, 계정, 인쇄,
  본문(검색·개인 메모), 그리고 **로그인 화면**(`overflow:hidden`+세로중앙이라 스크롤 자체가
  불가능했음 → 키보드 시 상단 정렬 + 스크롤 허용).
- 플러그인 바인딩이 늦을 경우를 대비해 `load`/`deviceready` 재시도 추가.

### ✅ 합산 — 시작/종료를 달력 한 번에 선택

- `<input type="date">` 두 개(시작일·마감일 따로)를 **인라인 범위 달력 하나**로 교체.
  첫 탭 = 시작일, 두 번째 탭 = 종료일(시작일보다 앞이면 시작일로 재설정), 사이 날짜는 하이라이트.
- 기존 `rangeFrom`/`rangeTo` 값은 hidden input으로 그대로 유지 → `calcRangeSum()`은 변경 없음.

### ✅ ±5일 창이 KST가 아니라 UTC 기준이던 문제

- `rosterWindow()`가 `toISOString()`으로 날짜를 잘라 **Workers 런타임(UTC) 기준**으로 계산됨.
- KST 00~09시에 가져오면 창이 하루씩 밀려서 **6일 전 비행이 딸려 나오고 +5일째가 빠짐**.
- → `+9시간` 보정 후 자르도록 수정. 로스터 날짜가 한국 날짜이므로 이제 정확히 앞뒤 5일.

### 검증 / 배포
- `npm test` 41개 통과, Worker 배포(`91e77e5e` → `2b4b6534`).
- **TestFlight 1.1(9) 업로드 완료** (Delivery UUID `c7f1fee3-4f66-4586-95c1-b81c03ce82aa`).
  테스트 그룹 연결은 사용자가 직접 진행.
